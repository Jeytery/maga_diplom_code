...
if ($roleAccessAllowed) {
    ...
    /*
       Finding tracks by type, name, tag of track_parts in shared tracks, route tracks, sensor tracks
!      ( `tracks`, `track_parts` WHERE `tracks`.track_id = `track_parts`.track_id AND `track_parts`.track_type_id = 1 )
X      ( `tracks` LEFT JOIN `track_parts` ON `tracks`.track_id = `track_parts`.track_id )
    $trackTypeId = getRequestValue('trackTypeId');
    if ($trackTypeId && isValidUUID($trackTypeId)) {
        $trackTypeId = getWhereParam('`track_parts`.track_type_id', false, 'UUID_TO_BIN( \'' . $trackTypeId . '\', true )');
    }
    */
    ...
    if ($userAccess != false && $userAccess->{PRIV_CAN_SELECT}) { // Admin access
        ...
        $hasGroupAccess = isUserAccessToRouteInGroup($currentUserId, $findRouteId);

        if ($userAccess != false && $userAccess->{PRIV_CAN_SELECT} || $hasGroupAccess) {
