...
/*
    Insert User with Google
    Url: POST /service/api/v1/user/register/google
         { "idToken": <idToken> }
...
if ($deviceType == IOS || $deviceType == ANDROID) {
    ...
    try {
        ...
        $googleUserId = $verifiedIdToken->claims()->get('sub');
...
$userAlreadyRegistered = $database->getIntValue("SELECT COUNT(*) FROM `object_options` oo WHERE oo.object_name = 'users' AND oo.name = 'googleUserId' AND oo.value = '$googleUserId'");

if ($userAlreadyRegistered == 1) {

    // Login
    $sqlRequest = "SELECT JSON_OBJECT ( 'userId', BIN_TO_UUID( u.user_id, true ), 'roleId', BIN_TO_UUID( u.role_id, true ), 'username', u.username, 'password', HEX(u.password), 'status', u.status, 'accessTokenCreated', UNIX_TIMESTAMP ( u.access_token_created ), 'accessToken', u.access_token, 'updateToken', u.update_token ) FROM `users` u, `object_options` oo WHERE u.user_id = oo.object_id AND oo.object_name = 'users' AND oo.name = 'googleUserId' AND oo.value = '$googleUserId'";
    ...
    if ($isCompleted) {
        ...
        // User option Registration with Google
        $registration = GOOGLE;
        ...
        // User option Google UserId
        $sqlRequest = "INSERT INTO `object_options` ( object_id, object_name, name, value, option_type )
        VALUES ( UUID_TO_BIN( '$userId', true ), 'users', 'googleUserId', '$googleUserId', 'string')";
