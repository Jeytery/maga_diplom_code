...
// Get the privileges
$objectName = 'routes';
getRoleAndPublicPrivileges($objectName);
...
if ($roleAccessAllowed) {
    ...
    // Public route Id List
    $publicRoutesIds = getPublicAccessToObjectIds($currentPublicObjectPrivileges, PRIV_CAN_SELECT);
    ...
    `routes`.route_id IN ($publicRoutesIds) AND `routes`.user_id != `users`.user_id" . $routeId . $routeTypeId . $name . $tag . $date . $userId . "
