...
if ($roleAccessAllowed) {
    ...
    if ($routeId && isValidUUID($routeId)) {
        $findRouteId = $routeId;
        $routeId = getWhereParam('route_id', false, 'UUID_TO_BIN( \'' . $routeId . '\', true )');
    } else {
        $routeId = "";
    }
    ...
    if ($routeTypeId && isValidUUID($routeTypeId)) {
        $routeTypeId = getWhereParam('route_type_id', false, 'UUID_TO_BIN( \'' . $routeTypeId . '\', true )');
    } else {
        $routeTypeId = "";
    }
    ...
    if ($date) {
        $date = getWhereExpression("started <= FROM_UNIXTIME(" . $date . ") AND finished >= FROM_UNIXTIME(" . $date . ")");
    }
    ...
    if ($userAccess != false && $userAccess->{PRIV_CAN_SELECT}) { // Admin access
        ...
        if ($routeId) { // Find by RouteId
            ...
            $userAccess = getUserAccessToObject($currentUserObjectPrivileges, $objectName, $findRouteId);
            ...
            $routesIds = getUserAccessIdsToObject($objectName, "can_select");
            if ($routesIds) {
                $routeId = getWhereExpression("route_id IN (" . $routesIds . ")");
            }

            // Seek the routes of the user
    ...
    if ($userAccessAllowed) {
        ...
        FROM `routes`" . $routeId . $routeTypeId . $name . $tag . $date . $userId . "
        ORDER BY route_id" . $sort . $offsetRows);
